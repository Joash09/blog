...
I planned for the hypervisor to run only the minimal tools required to spin up and manage virtual machines. To spin up VMs I opted for QEMU with KVM backend. To manage storage, I opted for LVM which allows to abstract storage away from disks so I can resize and snapshot partitions on the fly. To manage the running VMs, I opted to use libvirt which is a tool provides a unified platform for administrating virtualized resources such as VMs created using different back-ends such as QEMU or VirtualBox. It also has functionality for administrating different types of containers such as Docker or LXD, as well as virtualized networking and storage.

Finally, a really important feature for me was being able to run the VMs without root access. It bothered me that VMs are typically run as root, and that a compromised VM with root access effectively has access to my entire system. Of course, will require privileged access for certain tasks, but I prefer if I could control those permissions myself through sudo. Running VMs as root is also a minor reason why I opted to ignore solutions such as Proxmox or TrueNAS. The major reason for my approach was exposure to inner workings behind the tooling for virtualized servers.

A second design decision, was for a non-graphical hypervisor. Display servers and desktop environments, in my opinion added unnecessary compute overhead to my already modest server. Secondly display managers add extra bloat and attack vectors to my system, but this is probably more of a minor issue.


### Hypervisor LVM setup {#hypervisor-lvm-setup}

In an earlier writeup, I explained why an initramfs is necessary if your home directory also exists on a LVM logical volume as well as how I'd written my own initramfs. For this homeserver, I am going to take the easy route and use the dracut tool to generate an initramfs, instead of writing it by hand. The process is straightforward as you can see in the following commands.
...
I have written the following script to setup the virtual networking as described. This file is script is located in _usr/local/sbin_ where all user specific but privileged custom scripts are typically located. To set the script to automatically run at startup, create a systemd service in /etc/systemd/system/networking.service as shown in the later code snippet. Remember to enable the service after creation.
...
The first VM installed was OPNSense which now acts as my home router and firewall. To install the VM, I made use of virt-install, a tool included in the libvirt package. A OPNSense install image (.img) can be downloaded from their website. The nano version makes sense for a headless setup. However, since virt-install (more specifically QEMU working behind the scenes) cannot just mount any format (i.e. .img), some preparation must be done first. Using QEMU's builtin image to qcow2 format converter convert the image, then the resulting qcow2 format can be mounted and installed (i.e. unpacked) to our desired LVM volume. Finally, also notice in command below, how the NIC devices are being passthrough to the VM.
...
### Initial interface rules {#initial-interface-rules}

As we have seen in the VM configuration process earlier, an interface maps directly to a physical or software based device through which network data flows through. A collection of interfaces makes up a network. Devices which connect interfaces together are bridges. A network switch is an example of a bridge. Two distinct networks (i.e. two distinct groups of directly connected interfaces) are they themselves through a gateway device. The gateway itself is connected to both networks through its own interface.

The VM's network interfaces must be assigned an IP address. Since the VM is acting as our router, one must be connected to the WAN (i.e. your ISP). This interface's IP address is determined by your ISP so check with them if it is assigned statically, dynamically with DHCP or through PPPoE. The interface which will connect our firewall/router VM to the rest of the home network will be configured with a static IP address since we want that to stay constant. OPNSense will automatically configure a DHCP server for our LAN interface, which means any other interfaces (i.e. devices) belonging to the same network as our LAN interface will obtain its IP address from this DHCP server. The OPNSense LAN interface also acts as the default gateway for any device connected to the LAN network.
...
Domain name resolution (DNS) is the protocol which converts human readable URLs into IP addresses which is used to locate servers across the world. DNS requests typically are done through your ISP or  country's DNS server and is one of the last internet protocols which remain unencrypted. This is a privacy issue for internet users because although an ISP can track which sites you are visiting since your router needs to request the IP addresses of the websites you visit. This information is enough to build a profile on you given your browser history. Thankfully, due to HTTPS the traffic thereafter between you and the website should remain encrypted.

Choice of a DNS server is an important security choice as much as it is a privacy one. A compromised DNS server may direct users to fake IP addresses constituting "man-in-the-middle" attacks. Furthermore, certain DNS servers offer user features such as ad blocking and adult content filtering by not providing IP addresses to requests by you or by websites on your behalf.

Ideally, we would like to run our own DNS server which we can control. OPNSense comes bundled with UnboundDNS which, like dnsmasq, is a DNS server which will run on our machine. We will begin by creating a

-   Encrypted DNS requests
-   Upstream DNS server
-   DNS blocklist

After setting up our own DNS server on the OPNSense instance, consider the DNS server list on your individual devices will ignore your DNS server instance. On Linux machines with systemd-resolved, this means updating your /etc/resolv.conf configuration file and restarting the service.

```bash
nameserver 192.168.1.1 # OPNSense
...
If you do not want to change the DNS settings for all your devices, you can instead enforce all traffic to use your DNS server.
...
### Selective routing to external Wireguard VPN {#selective-routing-to-external-wireguard-vpn}
...
-   Transfer trust of who gets to monitor your network traffic from your ISP to an external 3rd party as well as mask your IP address to themselves. It is important to consider any VPN providers can still monitor, sell or leak your network data.
...
3.  Ensure the instance is configured with "disabled routes." Routing decisions will be made via a Wireguard Gateway which we will create later. The gateway IP address can be whatever we choose. It is recommended one number below the our IP address as provided by the VPN provider to avoid conflicts
...
6.  Create Wireguard peer with the details on one of the VPN provider's servers (e.g. South Africa). You will the external VPN's server public key and IP address of the server
...
Currently, I was only concerned with my IPv4 network.
NAT is specifically for IPv4, to preserve the limited number of IP addresses available to us. NAT is not a firewall!


### Assigning a public domain name and Dynamic DNS (DNS) {#assigning-a-public-domain-name-and-dynamic-dns--dns}

At this point we should begin to think about how to securely expose services hosted on our server such that they can be accessed remotely. Before tackling this we must consider how your connection with your ISP works using IPv4 (the version of the IP protocol still widely used today). We have already run out of avaialable IPv4 addresses. To overcome this, engineer implemented Network Address Translation (NAT). NAT introduced private and public IP addresses. In this setup, all your local devices with have a private IP address managed by you. Then an ISP will provide you with a single IP address. All traffic exiting the home network will substitute the source address with the ISP's public address and when receiving data the destination address is swapped back to the private address. A home network's private IP address space is independent of their neighbors. Thus the two homes can freely use private IP addresses, allowing us to save IP some addresses.
...
It is difficult to host services behind CGNAT, since the only IP address assigned to you is a private address only known to your ISP. Devices an external networks will not be able to reach it. To overcome this it is possible to request a public IP address from your ISP such that remote devices can reach me. If your ISP cannot provide you with a public IP address, you may look into hosting your services with an IPv6 service (the are exponentially more IP addresses with IPv6 and hence NAT is not necessary) or establishing a VPN tunnel from within your network by connecting to a Virtual Private Server (VPS) or Cloudflare's Argo tunnel. Once a connection is established, the VPN server can route traffic to you.

The public IP address your ISP provides to you can be either static or dynamic (obtained through DHCP). Static IP addresses are ideal for hosting service, however, ISPs charge extra for the privileged. I had to settle on a dynamic public IP. Given, I don't know when my IP address would change, I bought a domain name which would stay constant for all my users and service configurations. I then setup a Dynamic DNS (DDNS) software tool which runs on OPNsense which continuously pokes a DDNS provider informing them from which IP address you are connecting from. This DDNS provider would then continuously update their DNS records so remote devices will always have the latest IP address assigned to my domain name.
...
1.  Create Wireguard server instance. This includes setting the tunnel address (which also is the address for the DNS server)
